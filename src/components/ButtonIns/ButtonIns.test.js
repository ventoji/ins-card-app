...
describe('ButtonIns component', () => {
  ...
  it('should be defined and be small with danger mode by default', () => {
    ...
    const { container, getByText, getByRole } = render(<ButtonIns label="Add"/>);
  ...
  it('should render correctly large size with primary mode', () => {
    const { getByRole } = render(<ButtonIns label="Add" mode="primary" size="large"/>);
  ...
  it('should submit content when clicked', () => {
    ...
    const { getByText } = render(<ButtonIns label="Add" onClick={mockOnClick}/>);
